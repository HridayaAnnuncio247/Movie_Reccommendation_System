...
class Movie:
	...
	"""
	This function calls the TMDB API. Sometimes connecting to the API can fail. This function takes care of failures
	by retrying after some time.

	str url: The TMDB url for the year, language and page number of the movies to be extracted.
	int retries: The number of times we want to retry connecting to the API.
	int backoff: (backoff * retry attempt number) is the time gap between consecutive retries.
	"""
		for attempt in range(retries):
			        try:
			            ...
			            if response.status_code == 429:
			                ...
			                print("Rate limited. Waiting for", wait,"secs...")
			                time.sleep(wait)
			                continue
	...
	def add_to_db(self, data):
		"""
		This function adds a row equivalent of data to the movies collection of the database db.

		dict data: a dictionary where each key and value of the row equivalent is mentioned.
		"""
	...
	def run_api(self, year_range, language, API_KEY):
		"""
		This function creates ids and adds movies to the database.
		list year_range: a list with 2 elements. The first elements is the start year and the 2nd element is
		                 the last year. Both are included when extracting movies.
		str language: Indicates the language of the movies we want to etract. ("en" for English, "hi" for Hindi)
		str API_KEY: The personal API_KEY generated by TMDB. 
		"""
